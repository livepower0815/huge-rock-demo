...
<script>
  ...
  export default {
    ...
    methods: {
      getCoupons(page = 1) {
        const vm = this;
        const api = `${process.env.VUE_APP_APIPATH}/api/tingwankuo/admin/coupons?page=${page}`;
        vm.isLoading = true;
      ...
      add_edit() {
        let api = `${process.env.VUE_APP_APIPATH}/api/tingwankuo/admin/coupon`;
        const vm = this;
        let httpMethod = 'post'
      ...
      deleteCoupon(id) {
        const vm = this;
        vm.isLoading = true;
        const api = `${process.env.VUE_APP_APIPATH}/api/tingwankuo/admin/coupon/${id}`;
